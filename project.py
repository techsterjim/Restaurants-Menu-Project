...
def gconnect():
    ...
    if stored_credentials is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Account is already connected.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def oauth2callback():
    ...
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        'client_secrets.json', scopes=SCOPES, state=state)
...
def showRestaurants():
    ...
    if 'username' not in login_session:
        return render_template('publicrestaurants.html', restaurants=restaurants)  # noqa: E501
    else:
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction(){alert('You cannot \
...
@app.route('/restaurant/<int:restaurant_id>/menu/new/', methods=['GET', 'POST'])  # noqa: E501
...
# Edit a menu item
@app.route('/restaurant/<int:restaurant_id>/menu/<int:menu_id>/edit', methods=['GET', 'POST'])  # noqa: E501
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction(){alert('You cannot\
        edit menu items on this restaurant. Please create your own.');}\
        </script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
...
# Delete a menu item
@app.route('/restaurant/<int:restaurant_id>/menu/<int:menu_id>/delete', methods=['GET', 'POST'])  # noqa: E501
...
def disconnect():
    if 'provider' in login_session:
        ...
        flash("You are not logged in! Please try again.")
        return redirect(url_for('showRestaurants'))
